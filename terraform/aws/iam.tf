...
# AWS managed policy — allows ECS to pull ECR images and write CloudWatch logs
resource "aws_iam_role_policy_attachment" "ecs_execution_managed" {
  role       = aws_iam_role.ecs_execution.name
  policy_arn = "arn:aws:iam::aws:policy/service-role/AmazonECSTaskExecutionRolePolicy"
}

# Allow ECS to read our specific secrets from Secrets Manager
...
# ── IAM: ECS Task Role ────────────────────────────────────────────────────────
# Used by the running application code itself (not ECS infrastructure).
# Add permissions here if the app needs to call other AWS services (S3, SQS etc.)
...
resource "aws_iam_role_policy" "github_actions" {
  ...
  policy = jsonencode({
    ...
    Statement = [
      ...
      # ECS — update service to trigger rolling deploy
      {
        Effect = "Allow"
        Action = [
          "ecs:UpdateService",
      ...
      # IAM — pass roles to ECS task definitions
      {
        Effect   = "Allow"
        Action   = "iam:PassRole"
        Resource = [
